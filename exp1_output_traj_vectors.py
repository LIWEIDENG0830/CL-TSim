...
test_data_path = "../t2vec-master/experiment/exp1/exp1-trj.t"
test_model_dir = "./log/1629808399.533925/"
test_model_type = "LSTM"
save_output_format = "./outputs_exp1/{}_{}_hiddensize_{}_batchsize_{}_bidirectional_{}_nlayers_{}_freeze_{}_porto.pkl"
max_len = 100
hidden_size = 128
batch_size = 128
bidirectional = 0
...
for test_model_path in Path(test_model_dir).iterdir():
    if not test_model_path.name.startswith("checkpoint_"):
        continue
    if test_model_path.name != "checkpoint_0039_hiddensize_{}_batchsize_{}_bidirectional_{}_nlayers_{}_freeze_{}.pth.tar".format(hidden_size,
            batch_size, bidirectional, n_layers, freeze):
        continue
    epoch = test_model_path.stem.split("_")[1]
    save_output_path = save_output_format.format(features_type, epoch, hidden_size, batch_size, bidirectional, n_layers, freeze)
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            ...
            elif features_type == "predictor":
                features = model.encode_by_predictor(trajs, trajs_len)
            elif features_type == "middle":
                features = model.encode_by_middle_layer(trajs, trajs_len)
            else:
                raise ValueError
